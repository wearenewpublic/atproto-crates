...
- **Observability.** To get `/metrics` + email, rebuild the PDS image with `--features clap,hickory-dns,smtp,metrics` (the default Dockerfile builds only `clap,hickory-dns`) and set `PDS_METRICS_BIND` / `PDS_EMAIL_SMTP_URL`+`PDS_EMAIL_FROM_ADDRESS`.
