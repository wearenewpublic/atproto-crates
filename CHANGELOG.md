...
- **`describe_scope` covers `repo:`, `blob:` and `rpc:`.** All three fell through to "request access
  ...
  signs a proxied call with the account's own key. The audience renders a bidirectionally-verified
  handle when there is one, matching how `space:` owners already rendered.

- **A corpus guard over the bundled permission sets.** It expands every permission in every vendored
  set and fails the build on the unknown-resource refusal, turning a warning nobody reads into a red
...
- A permission whose expansion produces a non-token string now grants **nothing**, rather than
  ...
  and it makes a permission either granted or refused-with-a-reason — which is what the consent
...
- **The OAuth consent screen described a permission set's `rpc` and `space` permissions as "not
  granted by this server" while the token it was about to issue granted them.**

  The screen's summary had its own arms for `repo` and `blob` and a fall-through naming every other
  resource as refused. That was true when written. Then `rpc` expansion landed, and `space` after
  it, and the screen went on saying the opposite of what the token did — in the one place this
  codebase already holds must not contain a lie.

  The summary is now generated by expanding the set through the same `scopes_for_permission` the
